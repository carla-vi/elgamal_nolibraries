...
bool expoAndMod(uint64_t res[], uint64_t data[], uint64_t expo[], uint64_t key[]) {
...
bool mexpoAndMod(uint64_t res[], uint64_t data[], uint64_t expo[], uint64_t key[], uint64_t m[]) {
    int32_t i, j, expo_len;
    uint64_t mod_data[18] = {0}, result[18] = {0};
    uint64_t temp_expo = 0;
    ...
    // Multiply by m and reduce modulo key
    uint64_t intermediate[18] = {0};
    modmult1024(intermediate, result, m, key);
    modbignum(result, intermediate, key, 16);
